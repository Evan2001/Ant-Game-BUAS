...
{  
	...
	while (!exitapp) 
	{
        ...
        /*
			player
		*/
        string x = to_string(game->Move_Choice);
        char const* mvch = x.c_str(); 
        string y = to_string(game->Player_Health);
        char const* plyr = y.c_str();
        string z = to_string(game->Computer_Health);
        char const* cpt = z.c_str(); 
        char FirstLine[50];
        char SecondLine[50];
        char ThirdLine[50];
        char FourthLine[50];
        strcpy(FirstLine, "You chose ");
		strncat(FirstLine, mvch, 50);
		strpcy(SecondLine,"You inflicted " );
		


		/*
        strcpy(SecondLine, "You inflicted ");
        strcpy(ThirdLine, "Your health is ");
        strcpy(FourthLine, "The computer's health is ");
        strncat(SecondLine, mvch, 50);
        strncat(ThirdLine, plyr, 50);
        strncat(FourthLine, cpt, 50);*/

		/*
			computer
		*/
		x = to_string(game->Computer_Choice);
		switch (game->Computer_Choice){
			case 1:
				y = to_string(game->Bite_Damage);
				break;
			case 2:
				y = to_string(game->Claw_Damage);
				break;
			case 3:
				y = to_string(game->Acid_Damage);
				break;
			case 4:
				y = to_string(game->Grab_Damage);
				break; 

		}
        plyr = y.c_str();
        z = to_string(game->Computer_Health);
        cpt = z.c_str(); 
        FirstLine[50];
        SecondLine[50];
        ThirdLine[50];
        FourthLine[50];
        strcpy(FirstLine, "Computer chose \n");
        strcpy(SecondLine, "Computer inflicted ");
        strcpy(ThirdLine, "Your health is ");
        strcpy(FourthLine, "The computer's health is ");
		
		//strncat(FirstLine, );
        strncat(SecondLine, stpd, 50);
        strncat(ThirdLine, plyr, 50);
        strncat(FourthLine, cpt, 50);


        surface->Print(FirstLine, 2, 10, 0xffffff);
        surface->Print(SecondLine, 2, 30, 0xffffff);
        surface->Print(ThirdLine, 2, 50, 0xffffff);
        surface->Print(FourthLine, 2, 70, 0xffffff);
