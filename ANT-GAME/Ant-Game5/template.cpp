...
{  
	...
	while (!exitapp) 
	{
		...
		t.reset();
		game->Tick( elapsedTime, game->CurrentOpponentNum );
		// event loop
		SDL_Event event;
		bool pressed =false;
		while (SDL_PollEvent( &event )) 
		{
			...
			case SDL_QUIT:
				exitapp = 1;
				break;
			case SDL_KEYDOWN: // input controls for the player
				if (event.key.keysym.sym == SDLK_ESCAPE) 
				{
					exitapp=1;
				}
				if (event.key.keysym.sym == SDLK_1) //Key 1
				{
					pressed = true;
					game->Move_Choice=1;
					game->PressOne();
				}
				if (event.key.keysym.sym == SDLK_2)//Key 2
				{
					pressed = true;
					game->Move_Choice=2;
					game->PressTwo();
				}
				if (event.key.keysym.sym == SDLK_3)//Key 3
				{
					game->Move_Choice=3;
					pressed = true;
					game->PressThree();
				}
				if (event.key.keysym.sym == SDLK_4)//Key 4
				{
					pressed = true;
					game->Move_Choice=4;
					game->PressFour();
				...
				//printng out the player menu
				cout<<"-->Turn"<< game->Turn<<endl;
				surface->Print("Pick a Move. <1> <2> <3> or <4>:", 2, 102,0xffffff);
				surface->Print("1. Stampede", 2, 122 , 0xffffff);
				surface->Print("2. Bite", 2, 142,0xffffff);
				surface->Print("3. Swipe", 2, 162,0xffffff);
				surface->Print("4. Acid", 2, 182, 0xffffff);

				//printing out the player health
				char* Plyrhp;
				Plyrhp = (char*)malloc(50);
				if(Plyrhp!=nullptr){
					strcpy(Plyrhp, "");
					strcat(Plyrhp, "HP: ");
					string p_health = to_string(game->Player_Health);
					strcat(Plyrhp, p_health.c_str());
					if(Plyrhp!=nullptr)
						surface->Print(Plyrhp, 270, 455, 0xff0000);
					free(Plyrhp);
				}

				//printng out the opponent health
				char* Ohp;
				Ohp = (char*)malloc(50);
				if(Ohp!=nullptr){
					strcpy(Ohp, "");
					strcat(Ohp, "HP: ");
					string o_health = to_string(game->CurrentOpponent->hp);
					strcat(Ohp, o_health.c_str());
					if(Ohp!=nullptr)
						surface->Print(Ohp, 470, 100, 0xff0000);
					free(Ohp);
				}


				int choice=0;
				//when we press a button... this happens
				if(pressed){
					if(game->CurrentOpponent->hp<=0){
						game->CurrentOpponent->lost = true;
						
						//prints out what enemy died
						char* opp= (char*)malloc(15);
						if(opp!=nullptr){
							strcpy(opp,"");
							cout<< game->CurrentOpponent->OpponentName<< endl;
							strcat(opp, game->CurrentOpponent->OpponentName);
							strcat(opp, "died.");
							if(opp!=nullptr){
								surface->Print(opp, 500, 500, 0xffffff);
							}
						}
						//randomizes who the next opponent is
						time_t t;
        				srand((unsigned) time(&t));				
						int a = rand()%5;
						int i=0;
						while(game->played[a] && i<5){
							a = rand()%5;
							i++;
						} 
						...
						//win screen
						if(i<5){
							game->CurrentOpponentNum= a;
							game->played[a] = true;
							game->CurrentOpponent = game->opponent[a];
						}else{
							cout<<"YOU WON!"<< endl;
							exitapp=1;
						} 
						...
						//couts what turn it is
						choice = game->AttackPlayer();
					cout<<"TURN: "<< game->Turn<<endl;
					pressed = false;
				} 
				
				//decides the turn for the enemy
				if(game->Turn==0){
					game->Turn=1;
					game->PrintChoice(game->Move_Choice);

				} 
				//prints what the enemy chooses. 
				else{
					game->Turn=0;
					char *name =(char*)malloc(50);
					if(name!=nullptr){
						strcpy(name, "");
						if(game->CurrentOpponent->OpponentName !=nullptr){
							strcat(name, game->CurrentOpponent->OpponentName );
							strcat(name, " chose ");
							if(game->CurrentOpponent!=nullptr && game->CurrentOpponent->DamageNames[choice]!=nullptr){
								if(name!=nullptr){
									strcat(name, game->CurrentOpponent->DamageNames[choice]);
									if(name!=NULL)
										surface->Print(name, 450, 120, 0xffffff);
								}
							}
						}
					}
